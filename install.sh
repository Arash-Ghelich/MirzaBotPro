...
function install_bot() {
    ...
    ZIP_URL=$(curl -s https://api.github.com/repos/Arash-Ghelich/MirzaBotPro/releases/latest | grep "zipball_url" | cut -d '"' -f 4)

if [[ "$1" == "-v" && "$2" == "beta" ]] || [[ "$1" == "-beta" ]] || [[ "$1" == "-" && "$2" == "beta" ]]; then
    ZIP_URL="https://github.com/Arash-Ghelich/MirzaBotPro/archive/refs/heads/main.zip"
elif [[ "$1" == "-v" && -n "$2" ]]; then
    ZIP_URL="https://github.com/Arash-Ghelich/MirzaBotPro/archive/refs/tags/$2.zip"
fi
...
function update_bot() {
    ...
    if [[ "$1" == "-beta" ]] || [[ "$1" == "-v" && "$2" == "beta" ]]; then
        ...
        ZIP_URL=$(curl -s https://api.github.com/repos/Arash-Ghelich/MirzaBotPro/releases/latest | grep "zipball_url" | cut -d '"' -f4)
    fi
    ...
    if [ -n "$INSTALL_SCRIPT_PATH" ]; then
        ...
        RAW_INSTALL_URL="https://raw.githubusercontent.com/Arash-Ghelich/MirzaBotPro/main/install.sh"
...
EOF"
    ...
    git clone https://github.com/Arash-Ghelich/MirzaBotPro.git "$BOT_DIR" || {
...
function update_additional_bot() {
    ...
    if ! git clone https://github.com/Arash-Ghelich/MirzaBotPro.git "$BOT_PATH"; then
